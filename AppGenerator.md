...
29. **Reflected XSS - Contact Form Preview (Server-Side)**: Add a `/contact` page with a form (name, email, subject, message) that POSTs to `POST /contact/preview`. The PHP backend renders an HTML confirmation page (Content-Type: text/html) with all form values embedded directly using string interpolation (`$name`, `$email`, etc.) without `htmlspecialchars()`. Unlike the other XSS vulnerabilities (which are DOM-based via `dangerouslySetInnerHTML`), this is a traditional server-side reflected XSS where the PHP backend generates HTML with unsanitized input. The endpoint uses `$_POST` (standard form encoding), outputs a full HTML page, and calls `exit()` to bypass the JSON router. Handle the route before the JSON router in `index.php` (same pattern as `/pi-callback`). Add a "Contact" link to the Navbar between "About" and the auth links.
...
27. **SSRF via Wine Import URL -> JWT Secret -> Admin Token Forgery** (CWE-918): Add a `POST /wines/import-url` endpoint that accepts a `url` parameter and fetches content using PHP's `file_get_contents()`. The function supports `file://` protocol, allowing an attacker to read local files. The vulnerable method `importFromUrl()` in `WineController.php` does not validate the URL scheme or restrict access to internal resources. When the fetched content is not valid JSON, it is returned as `raw_content` in the response. An attacker chains: (1) SSRF to read `file:///var/www/backend/api/config/jwt.php`, (2) extract the hardcoded JWT secret from the PHP source, (3) forge a properly HS256-signed JWT with `is_admin=true`, (4) access all `/admin/*` endpoints with a cryptographically valid token. Unlike chain #23, this produces a properly signed token that survives fixing the JWT `alg=none` and signature bypass bugs.
28. **SQL Injection -> TOTP Secret Extraction -> 2FA Bypass -> Account Takeover** (CWE-89, CWE-308): This chain uses existing SQL injection vulnerabilities (#2, #3, or #4) to extract `totp_secret` alongside `email` and `password_hash` from the users table. The attacker then cracks the weak password hash offline, uses the stolen TOTP shared secret to generate valid 2FA codes, and logs in as the victim with full 2FA bypass. This demonstrates that storing the TOTP secret in the same database as credentials nullifies 2FA when SQL injection is present.
